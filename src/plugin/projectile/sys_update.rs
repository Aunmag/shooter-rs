...
pub fn on_update(
    ...
    for (entity, mut projectile, mut transform) in projectiles.iter_mut() {
        if projectile.stopped {
            ...
            if projectile.config.is_rocket {
                let friendlies = projectile
                    .shooter
                    .and_then(|e| obstacles.get(e).ok())
                    .map(|e| e.3.config.kind);

                commands.add(Explode::new(projectile.calc_data(t0).0, friendlies));
            }
        ...
        {
            // do not do double damage (hit) with rocket, since they explode
            if !projectile.config.is_rocket {
                let contact_velocity = find_contact_velocity(
                    contact_position,
                    head,
                    tail,
                    head_velocity,
...
fn find_obstacle(
    ...
    for (entity, collision, transform, actor) in obstacles.iter() {
        ...
        if obstacle.is_close(contact, collision.radius) {
            ...
            if result.is_none_or(|o| o.3 > tail_distance) {
