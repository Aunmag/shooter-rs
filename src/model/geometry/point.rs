...
impl GeometryProjection<Line> for Vec2 {
    ...
    /// NOTE: Line direction must be normalized
    fn project_on_clamped(self, l: &Line) -> Vec2 {
        return self.project_on(l);
    }
}
...
impl GeometryProjection<LineSegment> for Vec2 {
    ...
    fn project_on_clamped(self, l: &LineSegment) -> Vec2 {
        let p = self;
        let v = l.0;
        let u = l.1;
        let l_length = l.length_squared();
...
mod tests {
    ...
    fn test_project_on_line_segment() {
        ...
        assert_eq!(
            Vec2::new(92.0, 82.0).project_on_clamped(&line).round(),
            Vec2::new(46.0, 62.0),
        );

        let line = (Vec2::new(1.0, 2.0), Vec2::new(1.0, 2.0));
        assert_eq!(Vec2::new(4.0, 77.0).project_on_clamped(&line), line.0);
    }
}
